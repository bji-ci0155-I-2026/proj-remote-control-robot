...
- **Extensión del Sistema (Asistencia Inteligente):** Como mejora al MVP, el robot integrará un modelo ligero de aprendizaje automático (**TinyML / Edge AI**) ejecutado de forma local. Mediante la **fusión de sensores** (multisensor), el sistema analizará el entorno en tiempo real para hacer recomendaciones de conducción, emitiendo alertas físicas (por ejemplo, sonoras) cuando identifique obstáculos, para advertir al usuario y evitar choques. Todo esto se procesará en la frontera, sin depender de la nube.
...
Es el sector de investigación y educación académica en robótica, enfocado específicamente en el estudio y desarrollo de dispositivos móviles con recursos limitados; con ruedas que requieran capacidades de navegación asistida procesadas localmente por medio de TinyML (Edge AI), que utilicen múltiples sensores para mejorar su precisión en decisiones para evadir riesgos.

**Caso de uso secundarios**
