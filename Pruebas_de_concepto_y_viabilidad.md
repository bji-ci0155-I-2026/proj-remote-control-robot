# Pruebas de Concepto y Viabilidad del Sistema Empotrado
...
El proyecto consiste en el diseño y desarrollo de un prototipo de robot móvil de dos ruedas adaptado para superficies planas, enfocado en la conducción asistida. A diferencia del planeamiento teórico inicial, el desarrollo del prototipo físico se consolidó en la siguiente especificación práctica:

- **Producto Mínimo Viable (MVP) Implementado:** Un sistema de teleoperación manual en tiempo real donde el robot es manejado a distancia mediante una interfaz móvil (Gamepad virtual) utilizando comunicación inalámbrica Bluetooth Classic nativa en la placa IdeaBoard.
- **Asistencia de Conducción Activa:** Para garantizar la seguridad del prototipo, se implementó un sistema de **fusión de sensores lógicos** de proximidad (frontal y trasero) que analizan el entorno en tiempo real. En lugar de ejecutar modelos pesados de TinyML en local (los cuales comprometían la tasa de respuesta en tiempo real debido a recursos limitados de memoria), el sistema utiliza un algoritmo heurístico optimizado basado en reglas de umbral de distancia. Esto permite emitir alertas físicas progresivas (sonido de frecuencia variable mediante un Buzzer) y forzar un **frenado automático e inmediato** (anulación de comandos de tracción) cuando se detecta un obstáculo a menos de 15 cm de distancia en la dirección del movimiento, evitando colisiones físicas sin depender de procesamiento externo.
...
También es de importancia para el sector de automóviles asistidos de pequeña y gran escala, al proveer un ejemplo de cómo se pueden utilizar los recursos del microcontrolador de la mejor manera para mantener el procesamiento local no bloqueante, lo cual hace más rápidas las decisiones, ahorrando milisegundos valiosos en sistemas donde la latencia determina el éxito de la evasión de un choque.
